...
class Question extends CI_Controller
{
			...
			if (empty($q_id)) {
				if ($this->form_validation->run() === true) {
					if ($this->question_model->create($postDataQuestion)) {
						...
						redirect('teacher/question/index',);
					} else {
						#set exception message
						$this->session->set_flashdata('exception', 'Please Try Again');
						redirect('teacher/question/create');
					}
				} else {
					#------------- Default Form Section Display ---------#
					$data['contents'] = $this->load->view("teacher/question/question_view", $data, true);
	...
	private function create()
	{
		...
		if (empty($q_id)) {
			...
			if ($this->form_validation->run() === true) {
				...
				#------------- Default Form Section Display ---------#
				$data['contents'] = $this->load->view("teacher/question/add_question_form", $data, true);
				$this->load->view("teacher/home/layout/main_wrapper_view", $data);
			}
		} else {
			/*-----------UPDATE A RECORD-----------*/
			if ($this->form_validation->run() === true) {
				...
				redirect('teacher/question/edit/' . $q_id);
			} else {
				#set exception message
				$this->session->set_flashdata('exception', 'Please Try Again');
				redirect('teacher/question/edit/' . $q_id);
	...
	public function fetch_questio_options()
	{
		...
		$this->load->view('teacher/option/view_option_ajaz', $data);
		//print_r($data);
	}
}
